 ...
 * 실패하면(백엔드가 아직 `/events/**`를 안 띄운 시기, 브리프 머리말) `onUnavailable`로
 * 상위 훅(`lib/use-events.ts`)에 넘겨 `lib/mocks/events.ts` 가짜 스트림으로 넘어간다 —
 * 그 뒤로도 계속 주기적으로 재시도해, 백엔드가 뜨면 자동으로 라이브로 돌아온다.
 */

import { API_BASE } from "./api";
import type { EventStreamStatus, WmsEvent } from "./types";
