/* ═══════════════════════════════════════════════════════════════════════════
   ...
   ★ 검수실의 모니터와 **같은 규칙**으로 만들었다 — 화면에 우리 프론트의 실제 화면
     (`/packing-win98`)을 축소해 그리고, 누르면 그 라우트로 넘어간다. 3D 안의 장비가
     이 시스템과 무관한 화면을 띄우고 있으면 "어딘가의 기계"로 끝나지만, 우리 화면이
     떠 있으면 창고와 대시보드가 한 시스템으로 읽힌다.

   ⚠️ 화면은 **그림**이지 실제 화면이 아니다. `/packing-win98` 의 레이아웃을 크게 고치면
...
/**
 * 포스기 화면 — `/packing-win98` 을 그대로 옮겨 그린다.
 *
 ...
 *   말하는 편이 정직하고, 원본과도 같다.
 *
 * ⚠️ 이건 **그림**이지 실제 화면이 아니다. `/packing-win98` 의 레이아웃을 크게 고치면
