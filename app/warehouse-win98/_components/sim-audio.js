...
const BGM_VOL = 0.34;
/* 시작 지점(초).
   ★ 0 이다 — **음원 파일 자체를 하이라이트 구간만 잘라 두었기 때문**이다. 원본 151초에서
     12초~47초 35초분만 남겼다(3.5MB → 820KB). 배포가  을 통째로 올리는 구조라,
     20초짜리 장면에 151초를 실어 보낼 이유가 없었다.
   ⚠️ 음원을 통짜로 갈아 끼우면 이 값을 다시 봐야 한다. 그때는 귀로 맞춰야 한다 — 파형만
      봐서는 "여기서부터가 좋다"를 알 수 없다. */
const BGM_START = 0;
/* ★ 1.2배속 (사용자 요청).
