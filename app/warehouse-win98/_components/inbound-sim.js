...
/* ── 시연용 상품 셋 ────────────────────────────────────────────────────────────
   ...
   ⚠️ 제약 ① **크레인이 없는 등급이 둘 있다.** D(대형)과 F(특대형)에는 스태커 크레인을
      ...
      실제로 고추장양념 4×4 16입(130.7), 사골곰탕 3열 6줄 18입(125.6)이 이 함정에 빠진다.
   ...
   ⚠️ 제약 ② 로봇은 통로를 **+x 로만** 간다(`routeTo` 주석 참고). 일감이 craneX 오름차순으로
      ...
      이라, A → B → C 로 가면 등록 순서(1·2·3)와 적재 순서가 맞아떨어진다.
...
export const DEMO_ITEMS = [
  /* 회차 1 — 스팸 1.81kg.
     단품 10.1×10.1×19.8 을 3×2 로 세워 담은 6개입 케이스
     30.3 + 20.2 + 19.8 = 70.3cm  ≤ 80  → A 극소형 (x -6.90, 뒷줄) */
  { sku: "CJ-SPM-06", name: "스팸 1.81kg 6개입", l: 303, w: 202, h: 198, model: "spam" },
  /* 회차 2 — 비비고 사골곰탕 500g.
     단품 15.6×5.8×21.8 을 2열 5줄로 세워 담은 10개입 케이스
     31.2 + 29.0 + 21.8 = 82.0cm  ≤ 100  → B 소형 (x +2.18, 앞줄) */
  { sku: "CJ-GMT-10", name: "비비고 사골곰탕 500g 10개입", l: 312, w: 290, h: 218, model: "gomtang" },
  /* 회차 3 — 백설 고추장삼겹살구이양념 2450G.
     단품 11.4×11.4×28.1 을 4×3 으로 세워 담은 12개입 케이스
     45.6 + 34.2 + 28.1 = 107.9cm  ≤ 120  → C 중형 (x +9.08, 뒷줄) */
  { sku: "CJ-SGS-12", name: "백설 고추장삼겹살구이양념 2450G 12개입", l: 456, w: 342, h: 281, model: "sauce" },
];
...
export function createInboundSim(THREE, deps) {
  ...
  /**
   * 시뮬레이션 전체 배속. 1 이 원래 속도다.
   *
   * ★ **마지막 물건이 칸에 들어가는 순간까지 15초**를 목표로 잡은 값이다 (사용자 요청).
   *   원래 속도에서 그 지점까지 약 19.5초가 걸리므로 19.5 / 15 ≈ 1.30 이다.
   * ⚠️ 이 값은 **재서 맞춰야 한다.** 위 19.5초는 계산으로 낸 근사값이고, 경로 길이·크레인
   *    위치는 등급마다 달라서 물품이 바뀌면 같이 달라진다. 맞추는 법:
   *      1. 한 번 돌려 마무리 카드의 시간(T)을 읽는다
   *      2. 마지막 적재는 그보다 약 1.6초 앞이다 (놓고 보여 주는 1.5초 + 포크 빼기)
   *      3. 새 값 = 지금 값 x (T - 1.6) / 15
   */
  const SIM_SPEED = 1.30;
  ...
  return {
    ...
    update(dt) {
      ...
      runT += dt;
      /* ★ 1.2배속 (사용자 요청). 속도 상수를 하나씩 곱해 다니는 대신 **시간을 빠르게 흘려**
           보낸다 — 주행·승강·포크·대기·자막 타이밍이 전부 한 숫자로 같이 당겨진다. 상수를
           따로 손대면 어디는 빨라지고 어디는 그대로여서 동작이 서로 어긋난다.
         ⚠️ `runT` 는 **곱하기 전에** 더한다. 마무리 자막의 소요 시간은 관객이 실제로 기다린
            시간이어야 한다 — 배속을 곱하면 화면은 20초 만에 끝났는데 자막에 24초라고 뜬다.
         ⚠️ 카메라 감쇠는 그대로 둔다(`warehouse-slot-3d` 쪽 실제 dt). 장면만 빨라지고
            시선은 같은 속도로 따라가야 흔들림이 안 늘어난다. */
      dt *= SIM_SPEED;
