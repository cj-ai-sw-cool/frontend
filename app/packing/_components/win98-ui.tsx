...
export function Panel({
  ...
  return (
    <section className={`${styles.raised} flex flex-col bg-[color:var(--surface)] p-2 ${className}`}>
      <h2 className={`${styles.titleText} mb-1 flex items-center justify-between gap-2`}>
...
export function TrayBox({
  ...
  tone?: "normal" | "error";
  /**
   * 배지 크기. 기본은 상태 표시줄용 작은 것(h-6)이고, `lg` 는 **손이 가는 줄**에 쓴다
   * (토트 스캔 바처럼 입력창·버튼과 같은 줄에 서는 경우).
   * ⚠️ 높이를 className 으로 덮어쓰게 두지 않는 이유: `h-6 h-9` 처럼 두 유틸리티가 같이
   *    들어가면 어느 쪽이 이길지가 클래스 문자열 순서가 아니라 **CSS 파일 순서**로 정해져
   *    예측할 수 없다. 골라 쓰는 값으로 두면 그 다툼 자체가 없다.
   */
  size?: "sm" | "lg";
}) {
  return (
    <div
      className={`${styles.sunken} ${styles.mono} flex items-center gap-2 bg-[color:var(--surface)] ${
        size === "lg" ? "h-9 px-3 text-[15px]" : `${styles.small} h-6 px-2`
      } ${
