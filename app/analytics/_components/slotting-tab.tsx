...
export function SlottingTab() {
   ...
   * before/after 를 쓴다(브리프 §1 "제안 없으면 evaluate만 현재"). `waveIds` 를 비우면
   * 서버가 최근 웨이브 20개를 고정으로 쓴다(백엔드 노트 §1.14, §3 "경로 평가") — 개수를
   * 고르는 매개변수가 없어 "평가 대상" Select 는 지금 화면 표시용일 뿐 요청에 반영되지
   * 않는다(백엔드 요청 후보로 남겨 둔다). */
  const evaluateRequest = useMemo(
    () => (activeProposalId === null ? { center } : null),
    [center, activeProposalId],
  );
  const evaluateQuery = useEvaluate(evaluateRequest);

  const heatmapByBay = useMemo(() => {
    if (!heatmapQuery.data) return null;
    return new Map(heatmapQuery.data.bays.map((row) => [row.bayId, row.lines]));
  }, [heatmapQuery.data]);

  const goldenBayIds = useMemo(() => {
    if (!goldenOn || !goldenZoneQuery.data) return null;
    return new Set(goldenZoneQuery.data.bays.map((row) => row.bayId));
  }, [goldenOn, goldenZoneQuery.data]);
