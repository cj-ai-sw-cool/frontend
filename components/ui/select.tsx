 ...
 *
 * 바뀐 것
 *   · SelectTrigger: `border` → `border-2`, `bg-transparent` → `bg-card`.
 *     입력 계열은 샘플에서 전부 흰 바탕 + 2px 검정 테두리다(489행).
 *     `dark:bg-input/30 dark:hover:bg-input/50` 는 뺐다 — --input 이 테두리색 전용이
 *     되면서 다크에서 흰 덩어리가 되기 때문. 대신 hover:bg-muted 로 통일했다.
 *   · SelectContent: 1px 링 + 흐릿한 블러 그림자(원본의 ring/shadow-md 조합) →
 *     `border-2` + 블러 0 하드 섀도우. 드롭다운은 화면 위에 **실제로 떠 있는** 표면이라
 *     카드와 달리 그림자를 붙였다(카드에 왜 안 붙였는지는 card.tsx 주석 참고).
 *   · SelectSeparator: h-px → h-0.5. 1px 선만 남으면 2px 테두리 사이에서 혼자 얇다.
 */
function Select({
  ...props
